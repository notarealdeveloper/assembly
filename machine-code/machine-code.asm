...
;
; F=machine-code && nasm -f bin -o $F $F.asm && chmod +x $F && ./$F ; echo $?
;
; Note: There's no linker step! We did that shit by ourselves.
;
; Even better, we use nasm with the -f bin flag, which means we actually
; *wrote* the assembly for a complete, runnable binary from scratch.
;
; I love this stuff. :P
;
; ===========================================================================
;
; Note: A lot of the background information can be found in "man elf"
;
; The length of the fields e_entry, e_phoff, and e_shoff is either 4 or 8 bytes (32 or 64 bits)
...
ehdr:                                   ; Elf32_Ehdr
    ...
    dw      ehdrsize                    ; e_ehsize :: Contains the size of this header, (52 bytes for 32 bit, 64 for 64 bit) (we're 52)
